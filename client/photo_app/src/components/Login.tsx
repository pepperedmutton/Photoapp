...
const Login: React.FC<LoginProps> = ({ setStatusMessage, setLoginToken, loginToken, handleLogout }) => {
  ...
  return (
    <div className={ loginToken.length > 0 ? "auth-form" : "auth-form" }>
      {loginToken.length > 0 ? (
        <button onClick={handleLogoutClick} className="navbar-btn" style={{ marginBottom: "1em" }}>
          Log out
        </button>
      ) : (
